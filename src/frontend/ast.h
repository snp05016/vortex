...
#include <string>
#include <variant>
// a constructor will create a new node with the location of the token, and the location of thee
// node will be be the location of the token. The location of the node will be used to report errors
// and warnings in the source code. a constrctor in C++ is a special function that runs everytime a
...
struct FieldAccessExpr : public Expr {
	...
	private:
		...
		// member variable to store the field name being accessed, string cuz only the name
		// of hte field is needed
