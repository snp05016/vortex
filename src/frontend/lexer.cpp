...
void Lexer::skip_blk_comment() {
  ...
  while (position + 1 < length &&
         !is_blk_comment_end(source[position], source[position + 1])) {
    position++;
  }
  if (position + 1 < length) {
    ...
    // finding */, we set position to length to
    // indicate EOF.
  }
}
Token Lexer::figure_out_token() {
  ...
  auto make_token =
      [&](TokenKind kind) { // lambda funct to quick create a token
  ...
  if (is_digit(current_char)) {
    ...
    bool is_float = false;
    if (position + 1 < length && source[position] == '.' &&
    ...
    if (is_float && position < length &&
        (source[position] == 'e' || source[position] == 'E')) {
      std::size_t exponent_position = position + 1;
      if (exponent_position < length && (source[exponent_position] == '+' ||
                                         source[exponent_position] == '-')) {
        exponent_position++;
      }
  ...
  if (is_string_literal_start(current_char) ||
      is_char_literal_start(current_char)) {
    const char closing_quote = current_char;
    const TokenKind literal_kind = is_string_literal_start(current_char)
                                       ? TokenKind::LIT_STRING
                                       : TokenKind::LIT_CHAR;
    position++;
    bool escaped = false; // to keep track of whether the last character
    // was an esp
    while (position < length) {
  ...
  if (position + 2 < length && source[position] == '.' &&
...
Token Lexer::next_token() {
  while (position < length) {
    ...
    if (position + 1 < length &&
        is_line_comment_start(current_char, source[position + 1])) {
      skip_comment();
      continue;
    }
    if (position + 1 < length &&
        is_blk_comment_start(current_char, source[position + 1])) {
      skip_blk_comment();
      continue;
    }
