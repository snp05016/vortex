...
Token Lexer::figure_out_token() {
		...
		if (is_digit(current_char)) {
				...
				if (is_float && position < length &&
				    (source[position] == 'e' || source[position] == 'E')) {
						...
						if (exponent_position < length &&
						    is_digit(source[exponent_position])) {
				...
				return make_token(is_float ? TokenKind::LIT_FLOAT
							   : TokenKind::LIT_INT);
		}
		...
		if (is_string_literal_start(current_char) || is_char_literal_start(current_char)) {
				...
				position++;
				bool escaped =
				    false; // to keep track of whether the last character was an esp
				while (position < length) {
		...
		if (is_operator(current_char)) {
				...
				if (Token token = check_two('-', '=', TokenKind::OP_MINUS_ASSIGN);
				    token.kind != TokenKind::INVALID_TOKEN)
						return token;
				if (Token token =
					check_two('*', '=', TokenKind::OP_MULTIPLY_ASSIGN);
				    token.kind != TokenKind::INVALID_TOKEN)
						return token;
