...
<p class="page-intro">Strings, arrays, structs and references in compiled programs. This is the stage where you decide, and write down, where every part of a value lives in memory.</p>
...
One item on that list is different from the rest. It is not a feature but a
document: "Define the memory layout for arrays, structs, and references." The
specification deliberately leaves the exact layout open. It says the exact
layout, alignment and ordering of arrays are "not yet part of the public v0.1
specification", and that a backend "must apply one documented layout
consistently" ([arrays, 7.8](../../specification/arrays.md#78-memory-and-layout)).
Structs get the same treatment in
[8.8](../../specification/structs.md#88-layout). So the language hands you the
decision, and in return asks you to write it down and stick to it.

This page explains what that decision covers. It does not make it for you.
...
contiguous
: Stored side by side with no gaps. The arrays chapter says arrays are
  "intended to use contiguous, predictable storage".
...
: Storing a two-dimensional array one whole row after another, so that
  neighbours in a row are neighbours in memory.
...
"does not promise" it uses exactly one bit. A `char` holds one Unicode
character and "is not defined as a one-byte value". `usize` is
"target-sized", so its size depends on the machine you compile for. Each of
those is a line in your layout document.
...
Two dimensions are where you have a real choice. Memory is a single row, but a
...
<text class="vx-text" x="40" y="218">The same array in memory, row-major (one possible choice)</text>
...
are in wide use in numerical software, and neither is wrong. Vortex has not
picked one, so you do, and the choice goes into your layout document.

The choice never changes what a program computes. `a[1, 2]` means row 1,
column 2, whichever way the bytes are arranged, and the spec requires the
backend to "preserve observable indexing behavior". What the choice changes is
speed. A processor reads memory in chunks, and reading neighbouring bytes is
much cheaper than jumping around. In matrix code, the innermost loop usually
...
work in the roadmap will care about. Record the choice now so that the
optimizer does not have to guess later.

Arrays of higher rank follow the same idea. A rank-3 array is a list of
rank-2 blocks, and your document says which index changes fastest as you move
through memory.
...
rejected in the front end.

Your layout may well store both as six numbers in a row. That is allowed. What
is not allowed is letting that coincidence leak back into the language, so
that one type is quietly accepted where the other was required.
...
numbers. The [constant evaluator](stage-5-types-and-rules.md) reduced `2 + 2`
to `4` before type checking compared shapes. The tour puts it plainly: code
...
128 bytes under four-byte `f32`. The arrays chapter also requires that
dimensions do "not cause the complete layout size to overflow", which means
the total byte count has to fit in the size type your target uses. That check
belongs before layout, but the layout step is the first consumer that really
depends on it, so make sure the two agree about the limit.
...
One array question is still open. Whether a dimension of zero is legal "remains
an open semantic decision", and until the specification chooses, an
implementation "should diagnose it as unsupported". Your layout document can
simply say that zero-length arrays never reach this stage.
...
<text class="vx-text-muted" x="330" y="62">Are fields kept in declared order?</text>
...
<figcaption>Figure 2. One possible layout for <code>Particle</code>: fields in declared order, each four-byte field at a multiple of four, and room at the end that may be padding. Every number here is a choice your layout document must pin down; the specification leaves offsets, padding and alignment open.</figcaption>
</figure>

The AST keeps the fields in declared order, and the structs chapter says so.
It then says that "physical field offsets, padding, alignment, and ABI
compatibility are not yet specified". Keeping declared order is the simplest
rule and makes the layout easy to predict. Reordering fields to save padding is
also a legitimate choice. Either way, **field access** becomes the same thing
...
layout, the combination needs no new decision.
...
- **Structs are values.** A struct expression "creates a value rather than an
  object with independent identity". How values are copied or moved at the
  machine level is left to you, "subject to preserving Vortex value behavior".
- **Empty structs are awkward.** `struct Marker {}` is grammatical, its storage
  is implementation-defined, and there is currently no way to write an
  expression that constructs one. The spec calls this an open issue. Your
  layout document can state a size for `Marker` without depending on the issue
  being settled.
...
pass arrays into functions without copying them." The spec's own example shows
why it matters.
...
mutable reference `&mut T` needs mutable storage, and a mutable reference may
not overlap with a conflicting access. Stage 8 does not re-check these rules,
but it must not weaken them. In particular, the code you generate may assume
that a `&mut` parameter does not overlap with anything else the function can
see, only because the front end has already guaranteed it.

Passing an array *without* a reference is also legal. A parameter of type
`[f32; 4]` receives an array value. The parameter grammar has no `mut`, and
bindings are immutable by default, so as the documents read today the callee
cannot write to such a parameter. That makes the difference between copying
the array and quietly handing over a hidden reference invisible to the
program. The choice is yours, and it goes in the layout document with
everything else.
...
- A written layout document: size, alignment and element or field offsets for every v0.1 type, following the roadmap's instruction to define the layout.
- One array order (row-major, column-major or another) chosen and recorded: the optimizer and every test depend on it.
...
- Shapes that arrive as plain integers from constant evaluation: code generation never evaluates a dimension at run time.
...
- References stored inside structs: v0.1 excludes reference fields with unbounded lifetimes.
- Matching the C ABI or any other outside layout: the spec says ABI compatibility is not yet specified.
- Layouts tuned for speed, such as tiled or blocked storage: that is post-v0.1 optimization work.
- Zero-length arrays: diagnose them as unsupported until the spec decides.
...
**Keeping the layout only in the code.** The spec asks for "one documented
layout". If the only record is the code generator, nobody can check it, and
...
**Treating nested and multidimensional arrays as the same type.** They may
share bytes. They do not share a type. The front end keeps them apart, and
