...
returns a value. The **constant evaluator** works out the values that must be
known before the program runs, which in v0.1 means the sizes of arrays.
...
compatible
: Allowed to be used together under the type rules. Vortex v0.1 has almost no
  automatic conversions, so compatible usually means "the same type".

mutable
: Allowed to change after it is created. In Vortex a variable is mutable only
  if it was declared with `mut`. The opposite is **immutable**.
...
: Working out the value of an expression while compiling, because the language
  needs that value before the program runs. Array sizes are the v0.1 case.
...
| `usize` | A size or index that fits the target machine | none, needs context |
...
Two entries need a note. `void` is not a value you can store. The spec rules it
out for local variables, parameter values, array elements and struct fields.
And array types carry their dimensions inside them, so `[f32; 4]` and
`[f32; 2, 2]` are different types even though both hold four numbers.
...
gets its type from its spelling, or from what the surrounding code demands.
Each operator then looks at the types of its children and decides two things:
whether it accepts them at all, and what type its own result has.
...
<text class="vx-text-muted" x="205" y="290">2 and 1 take the default, i32</text>
...
types chapter is blunt about this: implementations "must not invent silent
lossy conversions". If the programmer wants a floating-point result, they
write the conversion themselves as `f32(count)`, and the checker, not the
parser, decides that `f32(...)` here is a conversion rather than a call to a
function named `f32`.

### Literals and the types around them

A whole-number literal with nothing around it becomes an `i32`. A decimal
...
([types chapter, section 4.4](../../specification/types-and-values.md)).

"Nothing around it" is the important phrase. In `let height: usize = 64;`, the
written type asks for a `usize`, and the literal `64` is allowed to become one
because 64 fits. The spec calls this contextual literal fitting, and it is the
only automatic type adjustment v0.1 has. In `let count: u32 = -1;` the context
asks for an unsigned number and the value is negative, so the program is
rejected.

The minus sign is its own operator, not part of the literal. The spec says the
positive literal "is checked together with its unary context". This matters at
the edges of a type's range, and it is a good place for a negative test and a
positive test side by side.
...
because `void` is not a storable type. Parameters, struct fields and function
return types are never inferred. They must be written.
...
operators and shifts need integers. `&&`, `||` and `!` need `bool`.
Comparisons need compatible values and always produce a `bool`, and v0.1
defines no ordering for strings or structs. The
...
each argument must be compatible with its parameter's type.
...
and a non-`void` function must not use a bare `return;`.
...
In Vortex, a variable cannot change unless it was declared with `mut`. This is
a **semantic error** rather than a type error, because the types are fine. The
...
mutable reference. The diagnostics chapter asks for the error to point at the
assignment target, with an optional note at the declaration that lacks `mut`.
That note is what turns a correct message into a helpful one.
...
error. And while a mutable reference is in active use, no other reference may
...
fn main() {
    ...
    let reader = &values;          // rejected: writer is still in use below
    writer[0] = 5.0;
    print(reader[0]);
}
```
...
The references chapter is honest that the full model is unfinished. There is
no written lifetime syntax, the analysis "across function calls, nested
aggregates, and separate array elements" is still open, and the chapter states
the policy for the gap: "a conservative rejection is preferable to unsound
acceptance". In plain words, when your checker cannot prove a use of references
is safe, it should reject the program. A checker that is too strict can be
relaxed later without breaking anyone's code. A checker that lets an unsafe
program through has broken the language's promise.
...
a value. The statements chapter turns that into a rule: a non-`void` function
"must return a compatible value on every reachable path".

A control-flow path is one possible route through the function body. Every
`if` splits the route in two. A `return` ends it. The rule says that no route
may reach the closing brace of the function without passing a `return`.
...
The word "reachable" in the rule hides a question that the specification has
not answered yet. Consider a function whose body is a loop with a condition of
`true` that returns from inside. No path actually reaches the end, but seeing
that requires the compiler to reason about the value of the condition. Whether
v0.1 expects that reasoning, or only the simple structural kind shown in
Figure 2, is an open decision. Until it is made, choose the simpler rule,
document it, and write tests that show which side of the line such programs
fall on.
...
syntax. The [arrays chapter](../../specification/arrays.md) then requires that
before the type is compared, laid out in memory, or turned into code, every
dimension has been proved to:

- have an integer type;
- be known at compile time;
- fit in a `usize` extent the compiler supports;
- not make the total size of the array overflow.
...
needs early. The architecture page draws its boundary: it computes "required
compile-time values" and must not "evaluate arbitrary runtime behavior".

Milestone 5 names four ways a dimension can fail, each with a
dimension-specific message. The diagnostics chapter puts all four in one
category, the **constant-evaluation error**.
...
resizable in v0.1. The diagnostics chapter asks for a message "explaining that
a fixed dimension depends on a runtime parameter", which is a much better hint
than a generic "expected constant".
...
gives the static part of that. Each index must have an integer type. The
number of indexes must match the number of dimensions, so `matrix[row]` on a
two-dimensional array is an error in v0.1, which has no partial indexing. And
when an index is a constant that is provably outside the array, the compiler
rejects the program:

```vortex
let values = [10, 20, 30];
let selected = values[3];      // rejected: index 3 is past the end
```

An index known only at run time is a dynamic rule. It is left for
[stage 9](stage-9-runtime-safety.md), which adds the bounds check to the
generated code.

### The zero-length array is an open decision

What about `[f32; 0]`? The roadmap does not answer. It says instead: "Decide
and document the zero-length-array policy before marking this milestone
complete; parsing must not decide that semantic rule." Until that decision is
made, the arrays chapter says a compiler "should diagnose it as unsupported".

The decision has to settle more than one yes-or-no. It must say:

- whether `[f32; 0]` is a valid type, and whether a zero is allowed in only
  some positions, as in `[f32; 0, 4]`;
- whether a repeat expression such as `[0.0; 0]` is valid, given that an
  element-list array can never be empty;
- what indexing such an array means, since every index would be out of bounds;
- whether a zero reached by calculation, such as `[f32; 4 - 4]`, is treated
  the same as a written `0`;
- which diagnostic category is used if zero is rejected.

Whatever the answer, it belongs in the semantic checker and in the
specification, not in the grammar. The parser should accept `[f32; 0]` exactly
as it accepts `[f32; 4]`.
...
| No bare return in non-`void` | `return;` in `-> i32` | type or semantic | `return 0;` |
| No value returned from `void` | `return 1;` in `main` | type or semantic | `return;` |
...
| No overlapping mutable use | the `writer` and `reader` example | semantic | read through `reader` after the last use of `writer` |
...
| Dimension known at compile time | `[f32; size]` | constant | `[f32; 4]` |
...
| Constant index is in bounds | `values[3]` on three elements | type | `values[2]` |

Some of these rows depend on choices the specification leaves to you. The
two return rows sit between chapters: the diagnostics chapter files a
mismatched return value under type errors, while the statements chapter calls
"return legality" a semantic check. The category for an out-of-bounds constant
index is not fixed either; the diagnostics
chapter notes that the same bounds rule can be caught at either compile time
or run time, and messages should "describe the language failure rather than"
the pass that found it. The reference rows depend on how much of the unfinished
aliasing model you implement. Record your choice next to the test.
...
- Local type inference from the initializer, with `i32` and `f32` defaults for
  bare literals: the types chapter requires both.
...
- The basic shared and mutable reference rules, rejecting what cannot be
  proved safe: the references chapter asks for conservative rejection.
- The every-path-returns check for non-`void` functions.
- Rejection of `break` and `continue` outside loops.
- Constant evaluation of every array dimension, with four separate diagnostics
  for runtime-dependent, non-integer, negative and overflowing sizes.
- A written, documented zero-length-array policy: the milestone is not
  complete without it.
...
- A complete numeric conversion table: the spec has not fixed one, so accept
  only the conversions it documents.
- A full ownership and lifetime system: the spec says the complete model "is
  not yet finalized".
...
checks rules the specification has already written down. Where the
specification says a question is open, as it does for zero-length arrays,
reachability, and parts of the reference model, the job is to choose the
cautious answer, write it down, and test it. It is not the place to design the
...
- the zero-length-array policy is written down in the specification or a
  linked decision record;
- all tests from milestones 0 to 4 still pass.

The roadmap also gives two dimension examples that must behave as stated:
...
**Deciding semantic rules in the parser.** It is tempting to reject
...
differ. By the spec, they are equal. Type equality for arrays has to wait for
constant evaluation.
...
**Checking a literal without its minus sign.** The smallest `i32` is written
with a unary minus in front of a literal that, on its own, is one too large
for `i32`. The spec asks for the literal to be checked together with its
unary context. Checking the bare literal first rejects a valid program.

**Confusing "ends with a return" with "returns on every path".** A function
whose last statement is `return x;` can still have an earlier branch that
falls out of a loop or an `if` without returning. The rule is about paths,
not about the last line.
...
**Accepting what you cannot prove.** For references, a missing check is worse
than an overly strict one. The spec says so. When in doubt, reject, and say in
the message that the pattern is not supported yet.
