...
void expect_tokens(std::string_view test_name, std::string_view source,
		...
		for (const ExpectedToken& expected : expected_tokens) {
				...
				if (actual.location.start > source.size() ||
					...
					source.size() - actual.location.start) {
						fail(test_name,
						     "token span falls outside the source text");
						return;
				}
				const std::string_view actual_text(
				    source.data() + actual.location.start, actual.location.length);
				if (actual_text != expected.text) {
						fail(test_name,
						     "token " + std::to_string(token_index) +
							 " has text \"" + std::string(actual_text) +
							 "\", expected \"" +
							 std::string(expected.text) + "\"");
				}
				if (actual.location.start < previous_end) {
						fail(test_name,
						     "token spans move backwards or overlap");
				}
