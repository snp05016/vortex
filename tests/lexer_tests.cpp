...
void expect_tokens(std::string_view test_name, std::string_view source,
  ...
  for (const ExpectedToken &expected : expected_tokens) {
    ...
    if (actual.kind != expected.kind) {
      fail(test_name, "token " + std::to_string(token_index) + " has kind " +
                          std::to_string(static_cast<int>(actual.kind)) +
                          ", expected " +
    ...
    if (actual.text != source.data() + actual.location.start) {
      fail(test_name, "token text pointer does not match "
                      "its source span");
    }
  ...
  if (eof.kind != TokenKind::EOF_TOKEN) {
    fail(test_name, "lexer produced an unexpected token after the "
                    "expected sequence");
  }
...
void test_keywords() {
  expect_tokens("keywords",
                ...
                "void bool char i32 u32 usize f32 f64 String true false name "
                "_value value2",
                {
                    {TokenKind::KW_FN, "fn"},
                    {TokenKind::KW_STRUCT, "struct"},
                    {TokenKind::KW_LET, "let"},
                    {TokenKind::KW_MUT, "mut"},
                    ...
                    {TokenKind::KW_U32, "u32"},
                    {TokenKind::KW_USIZE, "usize"},
                    {TokenKind::KW_F32, "f32"},
                    {TokenKind::KW_F64, "f64"},
                    {TokenKind::KW_STRING, "String"},
                    {TokenKind::LIT_TRUE, "true"},
                    {TokenKind::LIT_FALSE, "false"},
                    {TokenKind::IDENTIFIER, "name"},
                    {TokenKind::IDENTIFIER, "_value"},
                    {TokenKind::IDENTIFIER, "value2"},
                });
}
void test_numbers() {
  expect_tokens("numbers",
                "0 42 0b0 0b1010 3.14 1.5e4 2.5E+8 9.0e-2 0..10 0..=10",
                {
                    {TokenKind::LIT_INT, "0"},
...
void test_operators_and_punctuation() {
  expect_tokens("operators and punctuation",
                ...
                "+= -= *= /= %= -> .. ..= ( ) { } [ ] ; . : ,",
                {
                    ...
                    {TokenKind::OP_LOGICAL_AND, "&&"},
                    {TokenKind::OP_LOGICAL_OR, "||"},
                    {TokenKind::OP_LOGICAL_NOT, "!"},
                    {TokenKind::OP_BITWISE_AND, "&"},
                    {TokenKind::OP_BITWISE_OR, "|"},
                    {TokenKind::OP_BITWISE_XOR, "^"},
                    {TokenKind::OP_BITWISE_NOT, "~"},
                    {TokenKind::OP_LEFT_SHIFT, "<<"},
                    {TokenKind::OP_RIGHT_SHIFT, ">>"},
                    {TokenKind::OP_PLUS_ASSIGN, "+="},
                    {TokenKind::OP_MINUS_ASSIGN, "-="},
                    {TokenKind::OP_MULTIPLY_ASSIGN, "*="},
                    {TokenKind::OP_DIVIDE_ASSIGN, "/="},
                    {TokenKind::OP_MODULO_ASSIGN, "%="},
                    {TokenKind::PUNC_ARROW, "->"},
                    {TokenKind::PUNC_RANGE, ".."},
                    {TokenKind::PUNC_INCL_RANGE, "..="},
                    {TokenKind::PUNC_LPAREN, "("},
                    {TokenKind::PUNC_RPAREN, ")"},
                    {TokenKind::PUNC_LBRACE, "{"},
                    {TokenKind::PUNC_RBRACE, "}"},
                    {TokenKind::PUNC_LBRACKET, "["},
                    {TokenKind::PUNC_RBRACKET, "]"},
...
void test_empty_input_and_stable_eof() {
  ...
  if (first.kind != TokenKind::EOF_TOKEN ||
      second.kind != TokenKind::EOF_TOKEN) {
    fail("stable EOF", "empty input did not repeatedly produce EOF");
  }
  if (first.location.start != 0 || first.location.length != 0 ||
      second.location.start != 0 || second.location.length != 0) {
